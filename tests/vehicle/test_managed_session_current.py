...
class TestManagedSessionCurrent(unittest.TestCase, VehicleTestHelper):

    # test successes
    def test_managed_session_current_success(self):
        VehicleObject = Vehicle(self.vin, mock=True, mute=True)
        _set_role(DEFAULT_TEST_LAMBDA_ROLE_NAME)

        try:
            result = VehicleObject.managed_session_current
